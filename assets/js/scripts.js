...
// Debug: confirm this script loads and how many accordion buttons are present
try {
    // debug logs removed
} catch (e) {
...
// Accordion buttons: use event delegation so buttons work even if added later
(function initAccordion() {
    if (window.__simpleAccordionInitialized) {
        return;
    }
    window.__simpleAccordionInitialized = true;
    function ensureInner(content) {
        ...
        // Reset padding on container and apply padding inside inner
    ...
    function toggleAccordion(button) {
        ...
        // Find the closest following element with .accordion-content
        ...
        // Ensure inner wrapper exists to preserve padding/margins
        var inner = ensureInner(content);
        ...
        if (isOpen) {
            ...
            if (inner) {
                ...
                var h2 = content.scrollHeight + 8;
                content.style.maxHeight = h2 + 'px';
            }
    ...
    document.addEventListener('click', function (e) {
        ...
        // Prevent accidental form submission if buttons are inside a form
        e.preventDefault();

        // Ensure native button elements don't act as 'submit' by default
        if (button.tagName === 'BUTTON' && button.type !== 'button') {
            try { button.type = 'button'; } catch (err) { /* ignore if readonly */ }
        }

        // click detected — toggling accordion
        toggleAccordion(button);
    });
    // Ensure any existing buttons have type="button" and initial ARIA attributes
    document.addEventListener('DOMContentLoaded', function () {
        var buttons = document.querySelectorAll('.accordion-button');
        buttons.forEach(function (btn) {
            if (btn.tagName === 'BUTTON' && btn.type !== 'button') {
                try { btn.type = 'button'; } catch (err) { /* ignore */ }
            }
            if (!btn.hasAttribute('aria-expanded')) {
                btn.setAttribute('aria-expanded', 'false');
            }

            // set aria-hidden on corresponding content
